...
namespace NorthwindML.Controllers
{
    public class HomeController : Controller
    {
        ...
        // return the path to a file stored in the Data folder of the website
        private string GetDataPath(string file)
        {
            return Path.Combine(webHostEnvironment.ContentRootPath, "wwwroot", "Data", file);
        }

        // create an instance of the HomeIndexViewModel class loaded with all the products
        ...
        {
            var model = CreateHomeIndexViewModel(); // create view model
            return View(model);
        }
        ...
        // create datasets for each country and then return to the default Index view.
        public IActionResult GenerateDatasets()
        {
            foreach (string country in countries)
            {
                IEnumerable<Order> ordersInCountry = db.Orders
                    .Where(order => order.Customer.Country == country) // filter by country to create different datasets
                ...
                IEnumerable<ProductCobought> coboughtProducts = 
                    ordersInCountry.SelectMany(order => 
                        ...
                        .Where(p => p.ProductID != p.CoboughtProductID) // exclude matches between a product and itself
                        .GroupBy(p => new { p.ProductID, p.CoboughtProductID }) // remove duplicates by grouping by both values
                        .Select(p => p.FirstOrDefault())
                        .OrderBy(p => p.ProductID) // make it easier for humans to read results by sorting
            ...
            return View("Index", model); // return to the Index view
        }
        ...
        public IActionResult TrainModels()
        {
            ...
            foreach (string country in countries)
            {
                ...
                IDataView dataView = mlContext.Data.LoadFromTextFile(
                    ...
                    columns: new[]
                    {
                        ...
                        // The key count is the cardinality i.e. maximum valid value.
                        // This column is used internally when training the model.
                        // When results are shown, the columns are mapped to
                        // instances of our model which could have a different
                        // cardinality but happen to have the same(?)
                        new TextLoader.Column(
            ...
            return View("Index", model); // return Index view
        }
        ...
        public IActionResult Cart(int? id)
        {
                ...
                GetDataPath("germany-model.zip")))
            {
                ...
                var predictionEngine = 
                    mlContext.Model.CreatePredictionEngine<ProductCobought, Recommendation>(modelGermany);
